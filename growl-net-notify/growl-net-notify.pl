...
#	version 0.2, version upgraded to support weechat 0.2.7+
#		also removed need for Parse::IRC
#
...
sub message_process_init {
	...
	if($weechat_version eq "0.2.6") {
		weechat::add_message_handler("weechat_highlight", "highlight_public_026");
		weechat::add_message_handler("weechat_pv", "highlight_privmsg_026");
	} else {
...
#
# 0.2.6 support for private messages, have to parse the IRC message
#
sub highlight_privmsg_026 {
    my ( $nick, $message ) = ( $_[1] =~ /:([^!]+)[^:]+:(.*)/ );
...
#
# 0.2.6 support for highlights of nicks in public, have to parse the IRC message
#
sub highlight_public_026 {
...
# we can now run this in 0.2.6 and 0.2.7 without issue
if(exists &weechat::info_get) {
	$weechat_version = "0.2.7";
	...
	weechat::hook_command("gl", "setup the growl notify script",
								  "on|off|setup [host] [password] [port]|inactive [time_in_seconds]|status|help",
								  "on: turn on growl notifications (default)\n"
								  ."off: turn off growl notifications\n"
								  ."setup [host] [password] [port]: change the parameters for registration/notification with Growl\n"
